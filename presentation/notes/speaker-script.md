...
| 02 Global load balancing | 21–32 | Trúc | 17 phút |
| 03 Algorithms | 33–43 | Khánh | 15 phút |
...
cái đều đứng độc lập được: phụ lục (44–48), rồi 32, 28, 18, 16. Đừng bao giờ bỏ slide 53 —
...
### 8 / 55 · Ba việc load balancer làm
...
### 13 / 55 · Packet không phải là request
...
### 14 / 55 · L4 load balancer chạy thế nào
...
### 15 / 55 · L7 load balancer chạy thế nào
...
### 16 / 55 · Request nằm ở đâu trong lúc chờ
...
fleet L7 phía sau lo route. Đúng cái hình EKS ở slide 32.
...
### 22 / 55 · Một region không giải quyết được gì
...
vào được. ③ *Sau khi connect*: đã nói chuyện với server rồi mới trả 302 bảo đi chỗ khác —
client phải mở lại nguyên một connection mới, nên chỉ xài cho vài trường hợp riêng. Phần lớn hệ
thống thật là ① hoặc ②, hoặc cả hai.
**Chốt:** Giữ cái bảng này trong đầu cho năm slide tới.

### 24 / 55 · DNS-based routing (GSLB)
...
### 25 / 55 · Vì sao DNS không bao giờ nhanh được
...
### 28 / 55 · HTTP redirection
**Nói:** Đòn bẩy ba, cái cuối. Lần này không đụng gì tới DNS hay BGP: chính code của mình trả
lời request đầu tiên bằng một cái `302`, header `Location: https://sg.shop.vn/`. Nhìn cái đếm ở
dưới: TCP, TLS, request tới redirector — ba round trip. Nhận 302 xong, client bỏ connection đó
và làm lại từ đầu với một hostname mới: DNS lookup mới, TCP, TLS, request — thêm ba round trip
nữa. Sáu thay vì ba, đúng cái giá mình thấy ở slide 22.
**→ 1:** Vậy trả giá đó để được gì? Redirector là code của mình, nên nó quyết định dựa trên thứ
DNS không bao giờ thấy. DNS chỉ thấy IP của resolver. Redirector thấy **account**: user này
đang ở Việt Nam, nhưng account của họ thuộc EU và data theo GDPR phải nằm ở Frankfurt — nên
gửi qua `eu.shop.vn` dù Singapore gần hơn. Nó cũng đọc được load hiện tại nếu mình
viết. Mấy mirror tải Linux chọn server đúng kiểu này.
**→ 2:** Cái bẫy: client chỉ hỏi **một lần**. Singapore chết, redirector biết ngay trong vài
giây — nhưng user này đã bookmark `sg.shop.vn` rồi, họ đi thẳng tới đó, redirector không được
hỏi lại lần nào nữa. Tệ hơn nữa nếu lỡ dùng `301`: trình duyệt cache nó không có hạn, lựa chọn
thành vĩnh viễn.
**Chốt:** Redirect một lần mỗi session, không phải mỗi API call. Dùng `302` hoặc `307`, không
bao giờ `301`. Và mỗi hostname theo region phải có failover riêng của nó — thường là chính DNS
hoặc anycast ở hai slide trước.

### 29 / 55 · Đòn bẩy nào, khi nào
**Nói:** Bảng so sánh. Failover: DNS mất phút, anycast mất giây, redirect thì chỉ có tác dụng với
lượt truy cập mới, còn edge proxy dưới một giây. Điều khiển theo từng request: không, không, chỉ
request đầu tiên, full L7. Thấy được load của backend: DNS và anycast không, redirect thì có nếu
mình tự viết, edge proxy có. Quy luật y hệt phần 1 — load balancer càng nằm *trên đường đi*, nó
càng làm được nhiều.
**→ 1:** Cái bẫy chung cho mọi đòn bẩy: health check toàn cầu khó thật sự. "ap-southeast-1 có khoẻ
...
### 34 / 55 · Load balancing algorithm là gì?
...
### 39 / 55 · IP hash khi số server thay đổi
...
### 40 / 55 · Sticky session: stateful vs stateless
...
- **"Tụi mình xài gRPC mà một pod ăn hết tải."** → Slide 32 cộng với 3.5: gRPC dồn mọi thứ lên
