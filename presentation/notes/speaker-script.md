...
- Keys: `→` / `←` step, `O` overview, `T` light theme, `S` sends live demo requests on slide 40.
- For section 4: `cd demo && npm install && npm start` in a second terminal, before the talk.
- Check slide 40 shows a green `live · <algorithm>` chip, not `recorded`.
...
| 01 Local load balancing | 10–15 | Trúc | 10 min |
| 02 Global load balancing | 16–20 | Trúc | 8 min |
| 03 Algorithms | 21–30 | Khánh | 15 min |
| 03 Appendix (only if time / Q&A) | 31–35 | Khánh | 0–8 min |
| 04 Demo | 36–41 | Khánh | 10 min |
| Thank you + Kahoot | 42 | both | 3 min |

Running long? Skip the appendix (31–35) and slide 20. Never skip 40 — the live demo is the
...
### 4 / 42 · One server fails on three separate axes
...
### 5 / 42 · Three servers, but every user still hits the first one
...
### 7 / 42 · Forward proxy, reverse proxy, load balancer
...
### 11 / 42 · L4 sees IPs and ports, L7 reads the request
...
### 15 / 42 · Who balances the balancer?
...
### 17 / 42 · DNS: return several IPs and let the client pick
...
### 22 / 42 · What is a load balancing algorithm?
...
### 27 / 42 · IP hash when servers change
...
### 28 / 42 · Sticky sessions: stateful vs stateless
...
### 31 / 42 · Divider — Beyond the four
**Say:** Two things worth knowing if you want to go further.

### 32 / 42 · Power of two choices
...
### 33 / 42 · One slow pod, four algorithms
...
### 35 / 42 · Consistent hashing, the ring
...
the interleaving we saw on slide 24.
...
### 41 / 42 · Why nobody saw the crash
...
server. If sessions lived in memory, they just got logged out. Which is exactly slide 28.

### 42 / 42 · Thank you + Kahoot
...
- **"Isn't the load balancer itself a single point of failure?"** → Slide 15. Short answer:
  yes, which is why it runs as a pair or a fleet, and why managed ones exist.
- **"Why not just use DNS round robin?"** → Slide 17. No health, no load, and TTL means a dead
  IP keeps getting traffic for minutes.
- **"We use gRPC and one pod gets all the load."** → Slide 13 plus 3.5: gRPC multiplexes
...
- **"Which algorithm should we use?"** → Slide 30. Start with round robin, move to least
  connections when request times vary, and treat sticky sessions as a smell.
- **"How fast is failover really?"** → Slide 19: detection plus TTL plus stragglers, so
