...
| 02 Global load balancing | 21–32 | Trúc | 15 phút |
...
cái đều đứng độc lập được: phụ lục (44–48), rồi 32, 18, 16. Đừng bao giờ bỏ slide 53 —
...
### 26 / 55 · Redirection
**Nói:** Đòn bẩy ba, và nó khác hẳn hai cái kia: xảy ra *sau khi* connection đã mở rồi. Entry
host `shop.vn` trả `GET /` bằng `302 Location: https://sg.shop.vn/…`. Trình duyệt resolve cái
tên mới đó rồi mở nguyên một connection khác — DNS, TCP, TLS, lại từ đầu.
**→ 1:** Cái nó có mà DNS không có: **toàn bộ request** — cookie, user ID, path. DNS chỉ thấy
IP của resolver. Nên nó pin được user vào đúng region giữ data của họ: data residency, tenant
pinning. `login.` của Salesforce đưa bạn tới đúng instance host; YouTube đưa bạn tới đúng một
`googlevideo.com` cache.
**→ 2:** Cái giá phải trả: request đầu tốn DNS + TCP + TLS **hai lần**. Entry host nằm ngay trên
đường đi, nên nó chết là không ai được redirect nữa. Dùng `307`/`308` cho non-GET — một `302`
có thể biến `POST` thành `GET` — và không ít API client, SDK không tự follow redirect.
**Chốt:** Chỉ dùng khi cái giá đó trả **một lần** và session ở yên: pin đúng lần đầu, mọi request
sau đi thẳng.

### 27 / 55 · Anycast — phụ lục, đi nhanh (~30 s)
...
### 28 / 55 · Anycast + edge proxy — phụ lục, đi nhanh (~30 s)
