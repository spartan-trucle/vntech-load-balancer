...
**Các loại LB** (theo ByteByteGo): xếp theo 3 câu hỏi — *cái gì chạy nó* (hardware: F5, Citrix · software: NGINX, HAProxy, Envoy · cloud: AWS ALB/NLB, GCP, Azure) · *nó đọc gì* (L4: IP + port, chọn theo connection · L7: path, header, cookie, chọn theo request) · *nó với tới đâu* (local: một cluster/region · GSLB: chọn region qua DNS/anycast). Một box thường là nhiều loại cùng lúc: ALB = cloud + software + L7.
...
- **2.2 Ba đòn bẩy (bảng có cột nhắc lại cơ chế mạng: DNS, anycast/BGP, HTTP 302; kèm hình nhỏ cho từng khoảnh khắc):** LB local là proxy *trên data path*; LB global phải tác động **trước khi có connection**. Name resolution (DNS — trước khi connect) · Routing (anycast/BGP — trong lúc connect) · Redirection (302 — sau khi connect, tốn thêm 1 RTT, hiếm dùng).
...
- **2.5 Redirection:** entry host `shop.vn` trả `302 Location: https://sg.shop.vn/…`, browser mở connection mới (DNS + TCP + TLS lần nữa). Cái được: thấy **toàn bộ request** — cookie, user ID, path — nên pin được user vào đúng region giữ data của họ (data residency, tenant pinning; ví dụ Salesforce `login.` → instance host, YouTube → một `googlevideo.com` cache cụ thể), điều DNS không làm nổi vì nó chỉ thấy IP của resolver. Cái mất: tốn DNS + TCP + TLS **hai lần** ở request đầu, entry host nằm trên đường đi nên nó chết là không ai được redirect nữa. Dùng `307`/`308` cho non-GET (`302` có thể biến `POST` thành `GET`), và nhiều API client/SDK không tự follow redirect. Chỉ dùng khi cái giá đó trả một lần và session ở yên.
- **Phụ lục · Anycast (đi nhanh):** nhiều PoP cùng announce một IP qua BGP. Failover = **rút announcement**, hội tụ trong vài giây, client không cần câu trả lời mới. Gotcha: BGP đếm **số network**, không phải ms; không chia được % traffic; cần ASN + IP block + peering nên đa số đi thuê.
- **Phụ lục · Anycast + edge proxy — câu trả lời thật (đi nhanh):** PoP là L7 proxy thật, bắt tay ngay tại chỗ (3 × 30 ms = 90 ms thay vì 690 ms) rồi đi 1 hop ấm qua backbone. Lấy lại **per-request control ở quy mô toàn cầu**: retry sang region khác, canary theo weight, drain cả một region. CDN = máy này + cache; lợi ngay cả khi **cache hit 0%**.
- **2.6 So sánh 3 đòn bẩy:** failover phút / giây / dưới giây · per-request control không / không / full L7 · thấy load không / không / có. Gotcha chung: **health check toàn cầu rất khó**, và khi nó flap thì nó chuyển cả một châu lục.
- **2.7 Kiến trúc 2 region:** Route 53 (latency + health, TTL 60 s) → NLB (L4, 3 AZ) → fleet proxy L7 (TLS, least request) → service.
- **2.8 Capacity:** failover chuyển traffic chứ **không chuyển capacity**. Mỗi region phải gánh được toàn bộ traffic, cộng dư để mất 1 AZ. Chạy tier LB ở 30–40%, rồi game-day drill để chứng minh.
- **2.9 Đường đi trên EKS:** Route 53 → ALB (L7) hoặc NLB (L4) → Ingress controller → (Service/kube-proxy cho call nội bộ) → Pod.
