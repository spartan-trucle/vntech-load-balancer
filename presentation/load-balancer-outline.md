...
- **2.7 HTTP redirection:** code của mình trả `302` + `Location: https://sg.shop.vn/` cho request đầu tiên; client bỏ connection, làm lại từ đầu với hostname mới → **6 round trip thay vì 3** (+ DNS lookup). Được: quyết định theo thứ DNS không thấy — account, home region (GDPR, Nghị định 53), load hiện tại; mirror tải Linux làm kiểu này. Bẫy: client chỉ hỏi **một lần** — bookmark `sg.shop.vn` thì region chết vẫn đi thẳng vào. Redirect 1 lần mỗi session, dùng 302/307, **không bao giờ 301** (browser cache vĩnh viễn), mỗi hostname region cần failover riêng.
- **2.8 So sánh các đòn bẩy (DNS · anycast · redirect · anycast + edge):** failover phút / giây / chỉ lượt truy cập mới / dưới giây · per-request control không / không / request đầu tiên / full L7 · thấy load không / không / nếu tự viết / có. Gotcha chung: **health check toàn cầu rất khó**, và khi nó flap thì nó chuyển cả một châu lục.
- **2.9 Kiến trúc 2 region:** Route 53 (latency + health, TTL 60 s) → NLB (L4, 3 AZ) → fleet proxy L7 (TLS, least request) → service.
...
- **2.11 Đường đi trên EKS:** Route 53 → ALB (L7) hoặc NLB (L4) → Ingress controller → (Service/kube-proxy cho call nội bộ) → Pod.
