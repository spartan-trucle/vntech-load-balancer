...
- **2.1, 2.3–2.7 global animations** (`02-global-load-balancing.html`, same `scene()` + `steps()`): 2.1 builds two rows (the balancer routing around a dead pod, then dying with the whole region — the point being it cannot route around itself → the 3 × 230 ms handshake ladder); 2.3 runs DNS lookups through a GSLB that answers and then drops out of the path (VN → SG, JP → Tokyo, then SG unhealthy so health filters before the policy); 2.4 lights up the five layers holding a stale answer, adds them into the failover formula, then replays the same five minutes with the old address still answering; 2.5 routes users through a BGP junction to the nearest PoP, withdraws an announcement, then shows BGP picking a distant PoP; 2.6 terminates the handshakes at the PoP, retries into a second origin, then serves from cache; 2.7 sends a first request to a redirector that answers 302 and the client starts over on the regional hostname (a round-trip tally counts 3 → 6), redirects by account home region rather than location, then has a region die while a user with the regional hostname bookmarked goes straight to it.
