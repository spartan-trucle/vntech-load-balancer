...
- **2.1, 2.3–2.5 global animations, plus the anycast appendix** (`02-global-load-balancing.html`, same `scene()` + `steps()`): 2.1 builds two rows (the balancer routing around a dead pod, then dying with the whole region — the point being it cannot route around itself → the 3 × 230 ms handshake ladder); 2.3 runs DNS lookups through a GSLB that answers and then drops out of the path (VN → SG, JP → Tokyo, then SG unhealthy so health filters before the policy); 2.4 lights up the five layers holding a stale answer, adds them into the failover formula, then replays the same five minutes with the old address still answering; 2.5 bounces a user off an entry host with a 302 to a regional hostname and pays for a second DNS+TCP+TLS connection, then redirects a second user by a tenant cookie DNS never sees, then kills the entry host so new users get nothing while already-redirected users keep going. Anycast and anycast + edge proxy are appendix slides now (eyebrow reads "Appendix ·", covered fast): anycast routes users through a BGP junction to the nearest PoP, withdraws an announcement, then shows BGP picking a distant PoP; the edge-proxy slide terminates the handshakes at the PoP, retries into a second origin, then serves from cache.
