...
- **4.6 live demo** (`04-demo.html`): uses `Charts.flow()` against the real NGINX in `../demo/` (`X-Upstream`, `X-LB`, `/__lb`, `/__health`). Falls back to a recorded round robin when `localhost:8000` doesn't answer. `flow()` is shared with section 3, so it lives in `charts-lib.js`.
